...
void ofQuickTimeGrabber::setDeviceID(int _deviceID){
	deviceID		= _deviceID;
	bChooseDevice	= true;
    cout<<" device set in qtvidgrabber\n";
}
...
//--------------------------------------------------------------------
vector<string> ofQuickTimeGrabber::returnDeviceNames(){
    return deviceNames;
}
//--------------------------------------------------------------------
    ...
    for(int i = 0 ; i < (*deviceList)->count ; ++i)
    {
        ...
        if(nameRec.flags != sgDeviceNameFlagDeviceUnavailable){
            ...
            for(int j = 0; j < numInputs; j++){
                ...
                string thisName="device["+ofToString(deviceCount)+"] "+p2cstr(pascalName)+p2cstr(pascalNameInput);
                deviceNames.push_back(thisName);
            ...
            string thisName="device["+ofToString(deviceCount)+"] "+p2cstr(pascalName);
            deviceNames.push_back(thisName+" (not available)");
            deviceCount++;
        }
    }
    printf( "-------------------------------------\n");
